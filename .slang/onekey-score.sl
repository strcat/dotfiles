...
#ifn$slrn_onekey_score_sl
putenv("slrn_onekey_score_sl=1");

#ifn$slrn_pl_sl
define load_sys_files(){ERROR_BLOCK{message("\nWymagane makro systemowe/Requi"+
"red system marco: slrn-pl.sl\n");}evalfile("slrn-pl.sl");}load_sys_files();
#endif
...
% error translations 4 slrn-pl (tsca@edb.dk, 20011122,20020418,20020422) %{{{
 variable komunikat = [
                  "Preference does not exist: %s",
                  "Opcja konfiguracyjna %s nie istnieje!",
                  "Wrong type for %s: This preference wants %s not %s",
                  "Parametr opcji %s powinien byæ typu %s, nie %s!",
                  "%s doesn\'t work in group mode!",
                  "%s nie dzia³a w oknie grup!",
                  "Unknown score type: %s",
                  "Nieprawid³owe kryterium punktowania: %s",
                  "Unknown score scope: %s",
                  "Nieprawid³owy zakres punktowania: %s",
                  "Ignoring subthread ...",
                  "Ignorujê podw±tek...",
                  "Watching subthread ...",
                  "Obserwujê podw±tek..." ]; reshape (komunikat, [7,2]);
% ---------------------------------------------------------------------- %}}}
...
{
   if (is_group_mode ())
     verror (komunikat[2,slpl->loc], _function_name());
   ...
   !if (is_substr("sfr",strlow(type)))
     verror (komunikat[3,slpl->loc], type);
   !if (is_substr("ta",scope))
     verror (komunikat[4,slpl->loc], scope);
